...
export function RegisterScreen({ navigation }) {
  ...
  async function handleRegister() {
    ...
    setError('');
    try {
      await api.post('/perfil/cadastro', { nome, email, senha });
      ...
      >
        <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center' }} keyboardShouldPersistTaps="handled">
          <View style={styles.container}>
            ...
            <Text style={styles.subtitle}>Junte-se a nós e comece a planejar</Text>
            ...
            <Card style={styles.cardForm}>
              ...
              <Pressable style={({ pressed }) => [styles.button, pressed && { opacity: 0.8 }]} onPress={handleRegister} disabled={loading}>
                {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Cadastrar</Text>}
              </Pressable>
            </Card>

            <Pressable onPress={() => navigation.goBack()}>
              <Text style={styles.link}>Já possui conta? Acesse aqui.</Text>
            </Pressable>
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
