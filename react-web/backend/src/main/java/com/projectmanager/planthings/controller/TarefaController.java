...
public class TarefaController {
    ...
    public ResponseEntity<List<TarefaResponse>> findAllByLista(
    ...
    @GetMapping("/me/{id}")
    public ResponseEntity<TarefaResponse> findById(@AuthenticatedPerfilId Long perfilId, @PathVariable Long id) {
        return ResponseEntity.ok(toResponse(tarefaService.findById(perfilId, id)));
    }

    @PostMapping("/me/lista/{listaId}")
    public ResponseEntity<TarefaResponse> save(
            @AuthenticatedPerfilId Long perfilId,
            @PathVariable Long listaId,
